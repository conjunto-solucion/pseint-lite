...
struct GConfig {
	...
	bool nassi_shneiderman = false; ///< usar diagramas de Nassi-Shneiderman en lugar de "cl�sico"
	bool shape_colors = true; ///< mostrar los bloques de diferentes colores
	bool dark_theme = false; ///< usar una combinaci�n de colores claros sobre fondo oscuro
...
struct GState {
	...
	bool selecting_entities = false; // para selecciones m�ltiples (rectangulares, shift+drag derecho)
...
struct GView {
	// zoom y panning
	int win_h = 0, win_w = 0; // tama�o de la ventana
...
// para asociar las lineas de c�digo al diagrama de flujo
...
void GlobalInitPre(); // inicializaciones globales que no pueden ser est�ticas	
void GlobalInitPost(); // inicializaciones globales que no pueden ser est�ticas	
