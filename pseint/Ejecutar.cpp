		...
		if (cadena[cadena.size()-1]==';') { // Si es una accion secuencial
			...
			if (instruction_type==IT_BORRARPANTALLA) {
				...
				for(size_t i=0, arg_num=1;i<cadena.size();++arg_num) {
					...
					_sub(line,string("Se eval�a la expresion: ")+aux1);
			...
			if (instruction_type==IT_DIMENSION) {
				do {
					...
					if (lang[LS_ALLOW_DINAMYC_DIMENSIONS]) { _sub(line,string("Se eval�an las expresiones para cada dimensi�n del arreglo ")+name); }
					for(size_t i=0;i<aux2.size();i++) {
			...
			if (instruction_type==IT_DEFINIR) {
				...
				while (tmp2<(int)cadena.size()) {
					...
					if (tipo==vt_numerica) {
						if (rounded) {
							_sub(line,string("Se define el tipo de la variable \"")+aux2+"\" como Num�rico(Entero).");
						} else {
							_sub(line,string("Se define el tipo de la variable \"")+aux2+"\" como Num�rico(Real).");
						}
						...
						_sub(line,string("Se define el tipo de la variable \"")+aux2+"\" como L�gico.");
					} 
				}
			} else
			// ------------- ESPERAR un tiempo --------------- //
			if (instruction_type==IT_ESPERAR) {
				...
				_sub(line,string("Se eval�a la cantidad de tiempo: ")+aux2);
				DataValue time = Evaluar(aux2);
				if (!time.CanBeReal()) ExeError(219,string("La longitud del intervalo debe ser num�rica."));
				else {
			...
			if (instruction_type==IT_ASIGNAR) {
				...
				// evaluar expresion
				_sub(line,string("Se eval�a la expresion a asignar: ")+aux2);
			...
			if (instruction_type==IT_REPETIR) {
				...
				_sub(line,"Se ejecutarán las acciones contenidas en la estructura Repetir hasta que");
				tipo_var tipo;
				...
				while (should_continue_iterating) {
					...
					if (should_continue_iterating)
						_sub(line_hastaque,string("La condición es ")+(valor_verdad?VERDADERO:FALSO)+", se conti�a iterando.");
				} while (should_continue_iterating);
				line=line_hastaque;
			...
			if (instruction_type==IT_PARACADA) {
				...
				for (int i=0;i<nelems;i++) {
					...
					_sub(line,aux1+" ser� equivalente a "+elemento+" en esta iteraci�n.");
			...
			if (instruction_type==IT_SEGUN) {
				...
				if (!val_control.CanBeReal()&&(lang[LS_INTEGER_ONLY_SWITCH]||!val_control.CanBeString())) {
					if (!lang[LS_INTEGER_ONLY_SWITCH]) 
						ExeError(205,"La expresión del SI MÚLTIPLE debe ser de tipo numerica o caracter.");
					else
						ExeError(206,"La expresión del SI MÚLTIPLE debe ser numerica.");
				}
