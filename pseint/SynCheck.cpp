...
static void SynCheckAux2(string &cadena, int &errores) {
	...
	for (int y=0;y<int(cadena.size());y++) {
		...
		string pre;
		{
			...
			if (pre.empty()) { SynError(317,string("Falta operando (antes de la condici�n coloquial ")+col.cond+")."); errores++; }
		...
		}
		{
			...
			if (col.binary) {
				if (y0>=y) { SynError(318,string("Falta operando (despu�s de la condici�n coloquial ")+col.cond+")."); errores++; }
			} else {
				if (y0<y) { SynError(319,string("No corresponde operando (despu�s de la condici�n coloquial ")+col.cond+")."); errores++; }
			}
...
//							cadena.erase(i,1); i--;
				...
				if (act==':' && instruction_type!=IT_OPCION) {SynError (226,"Operador no v�lido (:)."); errores++;}
				...
				if (ret!=w_no_op) {
					...
					if (i==0 or (act!=cadena[i-1])) { // si encuentra algo como "???" que marque un solo ?
						SynError (68,string("Caracter no v�lido (")+string(1,act)+")."); errores++;
					}
...
int SynCheck(int linea_from, int linea_to) {
	...
	int errores=0; // Total de errores , y cant hasta la instrucci�n anterior
	int flag_pyc=0, tmp;
	...
	for (int x=linea_from;x<programa.GetRefPoint();x++){
		...
		// Ignorar lineas de comentarios
		{
			...
			// poner un espacio al final para evitar casos especiales cuando no hay punto y coma y la instrucci�n es una sola palabra (ej: borrarpantalla; finalgo )
			if (len>2 && cadena[len-1]==';' && cadena[len-2]!=' ') { cadena.insert(len-1," "); len++; }
			else if (len>1 && cadena[len-1]!=';') { cadena+=" "; len++; } 
			
			// extraer la primer palabra clave para ver qu� instrucci�n es
			...
			if (first_word=="ENTONCES") {
				...
				int flag_segun=0;
				if (!bucles.empty()) {
					// comentado el 20121013 para que siempre diga que lo que esta antes del : es una instrucci�n y lo que esta despues otra.
//					if (bucles.back()==IT_SEGUN) { // si esta en segun comprobar la condici�n
						...
						if (pos_dp!=-1 && cadena[pos_dp+1]!='=') {
							...
							if (bucles.back()!=IT_SEGUN) { SynError (241,"Opci�n fuera de SEGUN."); errores++; }
						}
//					}
				}
				if (flag_segun==0) {
...
//						(cadena[i]>='a'&&cadena[i]<='z') || // no deberia hacer falta a esta altura
					...
					if ((not cadena.empty()) and lang[LS_LAZY_SYNTAX] and instruction_type!=IT_ASIGNAR) { // definici�n de tipos alternativa (x es entero)
...
//					}
			...
			// si entro en segun comprobar que haya condici�n
			if (!bucles.empty()) {
			...
			if (!in_process && instruction_type!=IT_NULL&&cadena!="") {
				SynError (43,lang[LS_ENABLE_USER_FUNCTIONS]?"Instrucci�n fuera de proceso/subproceso.":"Instrucci�n fuera de proceso."); errores++;
			}
			...
			if (instruction_type==IT_DEFINIR) {  // ------------ DEFINIR -----------//
				if (cadena=="" || cadena==";") {SynError (44,"Faltan par�metros."); errores++;}
				else {
					if (cadena[cadena.size()-1]!=';') {
						cadena=cadena+";";
					...
					if (!RightCompare(cadena," COMO REAL;") && !RightCompare(cadena," COMO ENTERO;") && !RightCompare(cadena," COMO CARACTER;") && !RightCompare(cadena," COMO LOGICO;") ) {
						if (!ignore_logic_errors) { SynError (46,"Falta tipo de dato o tipo no v�lido."); errores++;}
					} else {
						...
						for (int tmp1=0;tmp1<(int)cadena.size()-largotipo+1;tmp1++) {
							...
							if (tmp1>0 && tmp1<(int)cadena.size()-1)
								...
								{SynError (47,"Se esperaba fin de expresi�n (fin de la instrucci�n, o coma para separar)."); errores++;}
			...
			if (instruction_type==IT_ESCRIBIR || instruction_type==IT_ESCRIBIRNL){  // ------------ ESCRIBIR -----------//
				if (cadena=="" || cadena==";") {SynError (53,"Faltan par�metros."); errores++;}
				else {
					if (cadena[cadena.size()-1]==';')
					...
					for (int last_i=0, i=0;i<(int)cadena.size();i++) {
						...
						if (i>0 && i<(int)cadena.size()-1) {
							...
							if (cadena[i]==' ' && cadena[i-1]!='&' && cadena[i-1]!='|'  && cadena[i+1]!='&'  && cadena[i+1]!='|')
								{SynError (54,"Se esperaba fin de expresi�n."); errores++;}
						}
			...
			if (instruction_type==IT_ESPERAR){  // ------------ ESCRIBIR -----------//
				if (cadena=="" || cadena==";") {SynError (217,"Faltan par�metros."); errores++;}
				else {
					string args = cadena;
					...
					if (!res.CanBeReal()) {SynError (219,"La longitud del intervalo debe ser num�rica."); errores++;} else {
						for (int tmp1=0;tmp1<(int)args.size();tmp1++) if (args[tmp1]==' ') {SynError (240,"Se esperaba una sola expresi�n."); errores++;}
					}
				}
			}
			if (instruction_type==IT_DIMENSION){  // ------------ DIMENSION -----------//
				if (cadena=="" || cadena==";") {SynError (56,"Faltan par�metros."); errores++;}
				else {
					if (cadena[cadena.size()-1]==';')
						cadena[cadena.size()-1]=',';
					else
					...
					for (int tmp1=0;tmp1<(int)cadena.size();tmp1++) {
						...
						if (tmp1>0 && tmp1<(int)cadena.size()-1)
							if (comillas<0 && cadena[tmp1]==' ' && cadena[tmp1-1]!='&' && cadena[tmp1-1]!='|'  && cadena[tmp1+1]!='&'  && cadena[tmp1+1]!='|')
							{SynError (57,"Se esperaba fin de expresi�n."); errores++;}
						if (tmp2==0 && cadena[tmp1]==',') { // comprobar validez
							str=cadena;
							str.erase(tmp1,str.size()-tmp1);
							str.erase(0,tmp3);
			...
			if (instruction_type==IT_LEER){  // ------------ LEER -----------//
				if (cadena=="" || cadena==";") { SynError (63,"Faltan par�metros."); errores++; }
				else {
					if (cadena[cadena.size()-1]==';')
						cadena[cadena.size()-1]=',';
					else
					...
					for (size_t i=0;i<cadena.size();i++) {
						...
						if (i>0 && i<cadena.size()-1) {
							if (lang[LS_LAZY_SYNTAX] && cadena[i]==' ') cadena[i]=',';
							if (cadena[i]==' ' && cadena[i-1]!='&' && cadena[i-1]!='|'  && cadena[i+1]!='&'  && cadena[i+1]!='|')
								{SynError (64,"Se esperaba fin de expresi�n."); errores++;}
						}
						if (parentesis==0 && cadena[i]==',') { // comprobar validez
			...
			if (instruction_type==IT_PARA){  // ------------ PARA -----------//
				str=cadena; // cortar instrucci�n
				if (str.find(" ",0)==string::npos) {SynError (70,"Faltan par�metros."); errores++;}
										...
										if (str.find(" ",0)==string::npos) {
											...
											if (!LeftCompare(str,"CON PASO ")) {
												if (str=="CON PASO")
													...
													{SynError (82,"Se esparaba CON PASO o fin de instrucci�n."); errores++;}
											} else {
												str.erase(0,9);
			...
			if (instruction_type==IT_PARACADA){  // ------------ PARA CADA -----------//
				str=cadena; // cortar instrucci�n
				if (str.find(" ",0)==string::npos)
				{SynError (70,"Faltan par�metros."); errores++;}
				if (!RightCompareFix(str," HACER")) {
					if (lang[LS_LAZY_SYNTAX]) { str+=" HACER"; cadena+=" HACER";}
			...
			if (instruction_type==IT_OPCION) {  // ------------ opcion del SEGUN -----------//
				...
				while ((p=PSeudoFind(cadena,',',i))!=-1) {
					...
					if (res.IsOk() && !res.CanBeReal()&&lang[LS_INTEGER_ONLY_SWITCH]) {
						SynError (203,"Las opciones deben ser de tipo num�rico."); errores++;
					}
					i=p+1;
				}
				cadena[cadena.size()-1]=':';
			...
			if (instruction_type==IT_ASIGNAR) {  // ------------ ASIGNACION -----------//
				...
				str.erase(str.find("<-",0),str.size()-str.find("<-",0));
				if (str.size()==0)
				{SynError (85,"Asignaci�n incompleta."); errores++;}
				else {
					...
					for (int y=0;y<(int)str.size();y++){ // comprobar que se un solo parametro
						...
						if (y>0 && y<(int)str.size()-1)
							...
							{SynError (87,"Se esperaba fin de expresi�n."); errores++;}
						if (comillas<0 && parentesis==1 && str[y]==',')
						{SynError (88,"Demasiados par�metros."); errores++;}
					}
					if (str.size()==3)
					{SynError (89,"Asignaci�n incompleta."); errores++;}
					else {
						str.erase(str.size()-1,1);
			...
			if (instruction_type==IT_SI){  // ------------ SI -----------//
				if (cadena=="")
					...
					str=cadena; // Comprobar la condici�n
				...
				for (int tmp1=0;tmp1<(int)str.size();tmp1++) {
					...
					if (tmp1>0 && tmp1<(int)str.size()-1) {
						// si encuentra un espacio (que no saco SynCheckAux3) es porque habia una instrucci�n despues del si, faltaba el "ENTONCES"
						if (comillas<0 && str[tmp1]==' ' && str[tmp1-1]!='&' && str[tmp1-1]!='|'  && str[tmp1+1]!='&'  && str[tmp1+1]!='|') {
							if (lang[LS_LAZY_SYNTAX]) {
								...
								SynError (91,"Se esperaba ENTONCES o fin de expresi�n."); errores++;
							}
						}
					}
				}
			...
			if (instruction_type==IT_HASTAQUE||instruction_type==IT_MIENTRASQUE){  // ------------ HASTA QUE -----------//
				...
				{ SynError (93,"Falta la condici�n en la estructura Repetir."); errores++; cadena+=" "; }
				else {
					str=cadena; // Comprobar la condici�n
					// comprobar que no halla espacios
					comillas=-1;
					for (int tmp1=0;tmp1<(int)str.size();tmp1++) {
						...
						if (tmp1>0 && tmp1<(int)str.size()-1)
							if (comillas<0 && str[tmp1]==' ' && str[tmp1-1]!='&' && str[tmp1-1]!='|'  && str[tmp1+1]!='&'  && str[tmp1+1]!='|')
							{SynError (94,"Se esperaba fin de expresi�n."); errores++;}
					}
					if (str[str.size()-1]==';') {
...
//				last_was_segun=true;
				...
				{ SynError (96,"Falta la variable/expresi�n de control en la estructura Segun."); errores++; }
					...
					if (RightCompareFix(str," HACER")) {
						...
						for (int tmp1=0;tmp1<(int)str.size();tmp1++) {
							...
							if (tmp1>0 && tmp1<(int)str.size()-1)
								if (comillas<0 && str[tmp1]==' ' && str[tmp1-1]!='&' && str[tmp1-1]!='|'  && str[tmp1+1]!='&'  && str[tmp1+1]!='|')
								{SynError (98,"Se esperaba fin de expresi�n."); errores++;}
						}
						DataValue res;
			...
			if (instruction_type==IT_MIENTRAS) { // ------------ MIENTRAS -----------//
				if (cadena==""||cadena=="HACER") { SynError (101,"Falta la condici�n en la estructura Mientras."); errores++; }
				else {
					if (RightCompare(cadena,";")) {
						if (!ignore_logic_errors) { SynError (262,"MIENTRAS no lleva punto y coma luego de la condici�n."); errores++; }
						cadena.erase(cadena.size()-1,1);
					}
					if (!RightCompareFix(cadena," HACER")) {
						if (lang[LS_LAZY_SYNTAX]) cadena+=" HACER";
							...
							if (tmp1>0 && tmp1<(int)str.size()-1)
								if (comillas<0 && str[tmp1]==' ' && str[tmp1-1]!='&' && str[tmp1-1]!='|'  && str[tmp1+1]!='&'  && str[tmp1+1]!='|') {
									if (lang[LS_LAZY_SYNTAX]) {
										...
										SynError (103,"Se esperaba fin de expresi�n."); errores++;
									}
								}
						}
						if (Lerrores==errores) {
			...
			if (fin_algo || instruction_type==IT_REPETIR || instruction_type==IT_BORRARPANTALLA || instruction_type==IT_ESPERARTECLA)
			{
				if (fin_algo && cadena==";") cadena="";
					...
					SynError (105,"La instrucci�n no debe tener par�metros."); errores++;
					cadena="";
				}
			}
			if (instruction_type==IT_ERROR && cadena!="" && cadena!=";") {
				if (LeftCompare(cadena,"FIN "))
					{SynError (99,"Instrucci�n no v�lida."); errores++;}
						...
						{SynError (106,"Instrucci�n no v�lida."); errores++;}
			}
			...
			if (instruction_type==IT_INVOCAR) {
				...
				if (func->GetTipo(0)!=vt_error && !ignore_logic_errors) {SynError (310,string("La funci�n retorna un valor, debe ser parte de una expresi�n (")+fname+")."); errores++;}
				...
				if (args=="();") {
					...
					if (args_last_pos!=-1) { // si faltaban cerrar parentesis, el error salto antes
						...
						else if (args_last_pos!=int(args.length())-2) {SynError (269,"Se esperaba fin de instrucci�n."); errores++;} // el -2 de la condici�n es por el punto y coma
					}
				}
			}
			// Controlar Cierre de Bucles
