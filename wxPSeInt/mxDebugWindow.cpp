...
//	sizer->Add(debug_status,wxSizerFlags().Proportion(0).Expand().Border(wxBOTTOM,10));
	...
	dp_check_step_in->SetToolTip(utils->FixTooltip(_Z("Cuando esta opci�n est� activada y el proceso llega a la llamada de una función entra en dicha funci�n y muestra pasa a paso c�mo se ejecuta la misma, mientras que si est� desactivada ejecuta la llamada completa en un solo paso sin mostrar la ejecuci�n de la misma.")));
	dp_check_step_in->SetValue(true);
	dp_check_backtrace=new wxCheckBox(this,mxID_DEBUG_BACKTRACE,_Z("Mostrar trazado"));
	dp_check_backtrace->SetToolTip(utils->FixTooltip(_Z("Cuando esta opci�n est� activada, durante la ejecución paso a paso aparece un panel en la parte inferior de la ventana que muestra qu� proceso fue invocando a qu� otro subproceso/funci�n hasta llegar al punto actual de la ejecuci�n.")));
...
#endif
	dp_check_subtitles->SetToolTip(utils->FixTooltip(_Z("Haga click en \"Comenzar\" para iniciar la ejecución paso a paso y leer en este panel los detalles de cada acci�n que realiza el int�rprete.")));
...
void mxDebugWindow::SetState(ds_enum state) {
	...
	case DS_STARTING:
		...
		dp_button_run->SetToolTip(utils->FixTooltip(_Z("Utilice este bot�n para detener definitivamente la ejecuci�n del algoritmo y abandonar el modo de ejecuci�n paso a paso.")));
		subtitles->text->SetValue(_Z("Haga click en \"Continuar\" para leer en este panel los detalles las pr�ximas acciones que realize el int�rprete."));
//		debug_status->SetLabel(_T("Iniciando"));
		dp_button_step->Disable();
		dp_button_step->SetBitmapLabel(dp_bmps[BMP_UN_PASO]);
		dp_button_step->SetToolTip(utils->FixTooltip(_Z("Utilice este bot�n para avanzar ejecutar solamente la siguiente instrucci�n del algoritmo.")));
	...
	case DS_STOPPED:
		...
		subtitles->text->SetValue(_Z("Haga click en \"Comenzar\" para iniciar la ejecuci�n paso a paso y leer en este panel los detalles de cada acci�n que realiza el int�rprete."));
		subtitles->button_next->Enable();
		dp_button_run->SetBitmapLabel(dp_bmps[BMP_COMENZAR]);
		dp_button_run->SetToolTip(utils->FixTooltip(_Z("Utilice este bot�n para que el algoritmo comience a ejecutarse autom�ticamente y paso a paso, se�alando cada instrucci�n que ejecuta, seg�n la velocidad definida en el men� configuraci�n.")));
		dp_button_step->SetBitmapLabel(dp_bmps[BMP_PRIMER_PASO]);
		dp_button_step->SetToolTip(utils->FixTooltip(_Z("Utilice este bot�n para ejecutar solo la primer instrucci�n del algoritmo y pausar inmediatamente la ejecuci�n del mismo.")));
...
//		debug_status->SetLabel(_T("No Iniciada"));
	...
	case DS_FINALIZED:
		...
		dp_button_run->SetToolTip(utils->FixTooltip(_Z("Ha finalizado la ejecuci�n del algoritmo. Utilice este bot�n para cerrar la ventana de la ejecuci�n del mismo.")));
		dp_button_pause->Disable();
		dp_button_step->Disable();
	...
	case DS_PAUSED:
		...
		dp_button_pause->SetToolTip(utils->FixTooltip(_Z("Utilice este bot�n para que el algoritmo contin�e avanzando paso a paso autom�ticamente.")));
	...
	case DS_RESUMED:
		...
		dp_button_pause->SetToolTip(utils->FixTooltip(_Z("Utilice este bot�n para detener temporalmente la ejecuci�n del algoritmo. Al detener el algoritmo puede observar el valor de las variables con el bot�n Evaluar.")));
		dp_button_evaluate->Disable();
