	...
	text(NULL), lang(cfg_lang)
{
	...
	sizer->Add(new wxStaticText(this,wxID_ANY,_Z(" Puede buscar por nombre de la instituci�n, materia, docente, siglas, etc.")),wxSizerFlags().Expand().Proportion(0).Border(wxTOP,5));
...
//	sizer->Add(new wxStaticText(this,wxID_ANY,_Z(" Descripci�n del perfil seleccionado:")),wxSizerFlags().Expand().Proportion(0));
...
//	sizer->Add(button_sizer,wxSizerFlags().Expand().Proportion(0));
	...
	this->Layout(); // para ajustar el tama�o de la columna de la lista
...
void mxProfile::UpdateDetails() {
	...
	if (lang.source==LS_FILE) {
		text->SetValue(wxString()
					   <<_Z("Su personalizaci�n actual ha sido cargada desde el archivo: ") << _S2W(lang.name)); 
		...
		text->SetValue(_Z("Puede utilizar el bot�n \"Personalizar\" para definir su propia configuraci�n.")); 
	}
}
...
static wxString normalize(wxString s) {
	s.MakeLower();
	s.Replace(_Z("�"),_T("a"),true);
	s.Replace(_Z("�"),_T("e"),true);
	s.Replace(_Z("�"),_T("i"),true);
	s.Replace(_Z("�"),_T("o"),true);
	s.Replace(_Z("�"),_T("u"),true);
	s.Replace(_Z("�"),_T("n"),true);
	s.Replace(_Z("�"),_T("u"),true);
	return s;
}
...
//	else text->SetValue(wxString()<<_Z("El perfil seleccionado actualmente (")<<lang.name<<_Z(") no aparece en esta b�squeda."));
}
